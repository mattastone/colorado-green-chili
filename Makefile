...
# `make all` to fetch transcripts and rebuild the map. `make retry` re-attempts
# IP-blocked videos after the rate-limit window has cleared.
...
.PHONY: help install fetch retry retry-all analyze map all serve clean clean-cache
